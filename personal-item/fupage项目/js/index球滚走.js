...
//      	console.log(juli);
        ...
        for (var i = 0; i < allPointLen; i++) {
			...
			colorPointAll[i].goX = p.x ;
			colorPointAll[i].goY = p.y ;
 ...
 //从起始点到3d圆球的位置，这期间不停的画各个小点；形成动画，过程中用一个状态监控这个动画是否完成，状态选择为：timerFace
    ...
    function drawOnePoint(i){
        ...
        ctx.arc(colorPointAll[i].x, colorPointAll[i].y, 0.5*scale , 0 , 2*Math.PI , true);
...
//		sphere.point[i].y = p.y * cos - p.z * sin;
//		sphere.point[i].z = p.z * cos + p.y * sin;
		
		var cos = Math.cos(angleX);
       	var sin = Math.sin(angleX);
       	for(var i=0;i< allPointLen;i++){
           	var y1 = colorPointAll[i].y * cos - colorPointAll[i].z * sin;
           	var z1 = colorPointAll[i].z * cos + colorPointAll[i].y * sin;
           	colorPointAll[i].y = y1;
           	colorPointAll[i].z = z1;
        }
	}
...
//		sphere.point[i].x = p.x * cos - p.z * sin;
//		sphere.point[i].z = p.z * cos + p.x * sin;
		
		var cos = Math.cos(angleY);
		var sin = Math.sin(angleY);
		for(var i=0;i< allPointLen;i++){
		    var x1 = colorPointAll[i].x * cos - colorPointAll[i].z * sin;
		    var z1 = colorPointAll[i].z * cos + colorPointAll[i].x * sin;
		    colorPointAll[i].x = x1;
		    colorPointAll[i].z = z1;
   		}
	}

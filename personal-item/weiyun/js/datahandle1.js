var dataAction = (function (){
    return {
    	...
    	//判断是否有文件类型的子集，处理是否添加图标
    	hasFileChilds:function (datas,id){
    		var newArr = dataAction.getChildsById(datas,id);
    		var onOff = false ;
    		for (var i = 0; i < newArr.length; i++) {
    			if (newArr[i].type == 'file') {
    				onOff = true ;
    				break;
    			}
    		}
...
//  				console.log(typeof datas[i].id ,typeof id);
    	...
    	//批量删除指定id下面的所有子集
    	//idArr 是一个id集合,里面是要删除数据的id
    	batchDelect:function (datas,idArr,recycleDatas){
    			...
    			for (var j = 0; j < childsAll.length; j++) {
	    			for (var k = 0; k < datas.length; k++) {
	    				if (childsAll[j].id == datas[k].id ) {
	    					var newArr = datas.splice(k,1);
	    					recycleDatas = recycleDatas.concat(newArr);
	    					break;
	    				}
	    			}
    			}
